...
@jax.jit
def log_rhat(x, θnum, θden, params):
    features = jnp.concatenate(jnp.broadcast_arrays(x, θnum, θden), axis=1)
    return mse_mlp.apply(params, features) - jnp.log(1e14)
...
def make_mse_loss(params, N):
    def mse_loss(params,rng):
        ...
        (log_r(xs, zs, thetarand, θsm) - 
         log_rhat(xs, thetarand, θsm, params)
        )**2
        )
...
params = mse_mlp.init(rng, features)
...
def gen_2d(nsamps):
    ...
    def nll_scan(theta):
        return -log_rhat(samp, theta, θsm, params).sum() 
