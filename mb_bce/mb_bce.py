...
mb_bce_mlp = mb_bce_MLP()

def sig(x, θnum, params):
    out = jnp.exp(mb_bce_mlp.apply(params, x))
    rhat = (1 + out[:, 0] * θnum[0] + out[:, 1] * θnum[1]) / (1 + θnum[0] + θnum[1])
    return rhat/(1 + rhat)
...
lr = 1e-3
epochs = 65000
params = mb_bce_mlp.init(rng, xsample)
...
for epoch in range(epochs):
    ...
    loss, grads = mb_bce_loss(params, rng)
